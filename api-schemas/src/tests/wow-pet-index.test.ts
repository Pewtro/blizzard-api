...
import { PetIndexResponseSchema } from '../generated';

test('wow.petIndex should match PetIndexResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['petIndex'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = PetIndexResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'wow.petIndex', err && (err as Error).message);
    throw err;
  }
});
