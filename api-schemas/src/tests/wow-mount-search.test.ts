...
import { MountSearchResponseSchema } from '../generated';

test('wow.mountSearch should match MountSearchResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['mountSearch'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = MountSearchResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'wow.mountSearch', err && (err as Error).message);
    throw err;
  }
});
