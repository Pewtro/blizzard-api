...
import { ItemTypeResponseSchema } from '../generated';

test('d3.itemType should match ItemTypeResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['itemType'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = ItemTypeResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'd3.itemType', err && (err as Error).message);
    throw err;
  }
});
