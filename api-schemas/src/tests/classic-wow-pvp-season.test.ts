...
import { PvpSeasonResponseSchema } from '../generated';

test('classic-wow.pvpSeason should match PvpSeasonResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['pvpSeason'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = PvpSeasonResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'classic-wow.pvpSeason', err && (err as Error).message);
    throw err;
  }
});
