...
import { SpellSearchResponseSchema } from '../generated';

test('wow.spellSearch should match SpellSearchResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['spellSearch'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = SpellSearchResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'wow.spellSearch', err && (err as Error).message);
    throw err;
  }
});
