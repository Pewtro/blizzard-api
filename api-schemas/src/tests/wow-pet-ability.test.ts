...
import { PetAbilityResponseSchema } from '../generated';

test('wow.petAbility should match PetAbilityResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['petAbility'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = PetAbilityResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'wow.petAbility', err && (err as Error).message);
    throw err;
  }
});
