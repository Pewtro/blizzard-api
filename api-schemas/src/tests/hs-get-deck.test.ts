...
import { GetDeckResponseSchema } from '../generated';

test('hs.getDeck should match GetDeckResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['getDeck'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = GetDeckResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'hs.getDeck', err && (err as Error).message);
    throw err;
  }
});
