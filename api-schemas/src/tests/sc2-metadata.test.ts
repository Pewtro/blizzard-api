...
import { MetadataResponseSchema } from '../generated';

test('sc2.metadata should match MetadataResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['metadata'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = MetadataResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'sc2.metadata', err && (err as Error).message);
    throw err;
  }
});
