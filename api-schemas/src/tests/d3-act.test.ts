...
import { ActResponseSchema } from '../generated';

test('d3.act should match ActResponseSchema', async () => {
  ...
  try {
    ...
    const parsed = ActResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'd3.act', err && (err as Error).message);
    throw err;
  }
});
