...
import { RecipeMediaResponseSchema } from '../generated';

test('wow.recipeMedia should match RecipeMediaResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['recipeMedia'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = RecipeMediaResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'wow.recipeMedia', err && (err as Error).message);
    throw err;
  }
});
