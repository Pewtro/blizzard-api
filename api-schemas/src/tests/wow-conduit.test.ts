...
import { ConduitResponseSchema } from '../generated';

test('wow.conduit should match ConduitResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['conduit'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = ConduitResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'wow.conduit', err && (err as Error).message);
    throw err;
  }
});
