...
import { ItemMediaResponseSchema } from '../generated';

test('classic-wow.itemMedia should match ItemMediaResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['itemMedia'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = ItemMediaResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'classic-wow.itemMedia', err && (err as Error).message);
    throw err;
  }
});
