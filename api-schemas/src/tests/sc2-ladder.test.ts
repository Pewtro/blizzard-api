...
import { LadderResponseSchema } from '../generated';

test('sc2.ladder should match LadderResponseSchema', async () => {
  ...
  try {
    ...
    const parsed = LadderResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'sc2.ladder', err && (err as Error).message);
    throw err;
  }
});
