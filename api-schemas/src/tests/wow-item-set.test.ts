...
import { ItemSetResponseSchema } from '../generated';

test('wow.itemSet should match ItemSetResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['itemSet'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = ItemSetResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'wow.itemSet', err && (err as Error).message);
    throw err;
  }
});
