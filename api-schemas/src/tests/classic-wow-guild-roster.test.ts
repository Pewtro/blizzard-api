...
import { GuildRosterResponseSchema } from '../generated';

test('classic-wow.guildRoster should match GuildRosterResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['guildRoster'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = GuildRosterResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'classic-wow.guildRoster', err && (err as Error).message);
    throw err;
  }
});
