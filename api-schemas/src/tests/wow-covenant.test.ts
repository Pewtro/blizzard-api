...
import { CovenantResponseSchema } from '../generated';

test('wow.covenant should match CovenantResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['covenant'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = CovenantResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'wow.covenant', err && (err as Error).message);
    throw err;
  }
});
