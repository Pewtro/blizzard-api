...
import { SpellMediaResponseSchema } from '../generated';

test('wow.spellMedia should match SpellMediaResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['spellMedia'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = SpellMediaResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'wow.spellMedia', err && (err as Error).message);
    throw err;
  }
});
