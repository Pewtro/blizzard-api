...
import { CovenantMediaResponseSchema } from '../generated';

test('wow.covenantMedia should match CovenantMediaResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['covenantMedia'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = CovenantMediaResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'wow.covenantMedia', err && (err as Error).message);
    throw err;
  }
});
