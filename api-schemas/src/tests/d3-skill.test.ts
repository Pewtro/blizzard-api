...
import { SkillResponseSchema } from '../generated';

test('d3.skill should match SkillResponseSchema', async () => {
  ...
  try {
    ...
    const parsed = SkillResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'd3.skill', err && (err as Error).message);
    throw err;
  }
});
