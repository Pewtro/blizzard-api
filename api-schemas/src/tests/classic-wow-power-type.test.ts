...
import { PowerTypeResponseSchema } from '../generated';

test('classic-wow.powerType should match PowerTypeResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['powerType'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = PowerTypeResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'classic-wow.powerType', err && (err as Error).message);
    throw err;
  }
});
