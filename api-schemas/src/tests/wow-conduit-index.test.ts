...
import { ConduitIndexResponseSchema } from '../generated';

test('wow.conduitIndex should match ConduitIndexResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['conduitIndex'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = ConduitIndexResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'wow.conduitIndex', err && (err as Error).message);
    throw err;
  }
});
