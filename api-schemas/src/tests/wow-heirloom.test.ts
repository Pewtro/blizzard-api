...
import { HeirloomResponseSchema } from '../generated';

test('wow.heirloom should match HeirloomResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['heirloom'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = HeirloomResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'wow.heirloom', err && (err as Error).message);
    throw err;
  }
});
