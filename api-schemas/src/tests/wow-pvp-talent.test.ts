...
import { PvpTalentResponseSchema } from '../generated';

test('wow.pvpTalent should match PvpTalentResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['pvpTalent'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = PvpTalentResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'wow.pvpTalent', err && (err as Error).message);
    throw err;
  }
});
