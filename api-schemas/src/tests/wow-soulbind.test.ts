...
import { SoulbindResponseSchema } from '../generated';

test('wow.soulbind should match SoulbindResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['soulbind'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = SoulbindResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'wow.soulbind', err && (err as Error).message);
    throw err;
  }
});
