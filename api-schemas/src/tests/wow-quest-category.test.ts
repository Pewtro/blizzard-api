...
import { QuestCategoryResponseSchema } from '../generated';

test('wow.questCategory should match QuestCategoryResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['questCategory'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = QuestCategoryResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'wow.questCategory', err && (err as Error).message);
    throw err;
  }
});
