...
import { MountResponseSchema } from '../generated';

test('wow.mount should match MountResponseSchema', async () => {
  ...
  try {
    ...
    const parsed = MountResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'wow.mount', err && (err as Error).message);
    throw err;
  }
});
