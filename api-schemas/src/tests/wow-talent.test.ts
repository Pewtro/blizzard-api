...
import { TalentResponseSchema } from '../generated';

test('wow.talent should match TalentResponseSchema', async () => {
  ...
  try {
    ...
    const parsed = TalentResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'wow.talent', err && (err as Error).message);
    throw err;
  }
});
