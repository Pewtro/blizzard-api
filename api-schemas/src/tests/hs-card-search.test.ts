...
import { CardSearchResponseSchema } from '../generated';

test('hs.cardSearch should match CardSearchResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['cardSearch'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = CardSearchResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'hs.cardSearch', err && (err as Error).message);
    throw err;
  }
});
