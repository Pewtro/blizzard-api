...
import { RegionIndexResponseSchema } from '../generated';

test('wow.regionIndex should match RegionIndexResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['regionIndex'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = RegionIndexResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'wow.regionIndex', err && (err as Error).message);
    throw err;
  }
});
