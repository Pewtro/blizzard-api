...
import { CreatureResponseSchema } from '../generated';

test('classic-wow.creature should match CreatureResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['creature'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = CreatureResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'classic-wow.creature', err && (err as Error).message);
    throw err;
  }
});
