...
import { QuestIndexResponseSchema } from '../generated';

test('wow.questIndex should match QuestIndexResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['questIndex'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = QuestIndexResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'wow.questIndex', err && (err as Error).message);
    throw err;
  }
});
