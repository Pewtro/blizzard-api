...
import { PvpTierMediaResponseSchema } from '../generated';

test('wow.pvpTierMedia should match PvpTierMediaResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['pvpTierMedia'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = PvpTierMediaResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'wow.pvpTierMedia', err && (err as Error).message);
    throw err;
  }
});
