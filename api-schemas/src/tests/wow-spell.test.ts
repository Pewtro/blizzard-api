...
import { SpellResponseSchema } from '../generated';

test('wow.spell should match SpellResponseSchema', async () => {
  ...
  try {
    ...
    const parsed = SpellResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'wow.spell', err && (err as Error).message);
    throw err;
  }
});
