...
import { LegacyLaddersResponseSchema } from '../generated';

test('sc2.legacyLadders should match LegacyLaddersResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['legacyLadders'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = LegacyLaddersResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'sc2.legacyLadders', err && (err as Error).message);
    throw err;
  }
});
