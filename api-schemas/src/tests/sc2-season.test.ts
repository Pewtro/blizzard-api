...
import { SeasonResponseSchema } from '../generated';

test('sc2.season should match SeasonResponseSchema', async () => {
  ...
  try {
    ...
    const parsed = SeasonResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'sc2.season', err && (err as Error).message);
    throw err;
  }
});
