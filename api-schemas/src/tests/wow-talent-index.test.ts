...
import { TalentIndexResponseSchema } from '../generated';

test('wow.talentIndex should match TalentIndexResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['talentIndex'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = TalentIndexResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'wow.talentIndex', err && (err as Error).message);
    throw err;
  }
});
