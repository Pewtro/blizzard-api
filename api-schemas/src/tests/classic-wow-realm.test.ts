...
import { RealmResponseSchema } from '../generated';

test('classic-wow.realm should match RealmResponseSchema', async () => {
  ...
  try {
    ...
    const parsed = RealmResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'classic-wow.realm', err && (err as Error).message);
    throw err;
  }
});
