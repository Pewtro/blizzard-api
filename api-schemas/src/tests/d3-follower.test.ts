...
import { FollowerResponseSchema } from '../generated';

test('d3.follower should match FollowerResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['follower'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = FollowerResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'd3.follower', err && (err as Error).message);
    throw err;
  }
});
