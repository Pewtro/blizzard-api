...
import { ProfessionResponseSchema } from '../generated';

test('wow.profession should match ProfessionResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['profession'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = ProfessionResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'wow.profession', err && (err as Error).message);
    throw err;
  }
});
