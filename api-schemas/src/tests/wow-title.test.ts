...
import { TitleResponseSchema } from '../generated';

test('wow.title should match TitleResponseSchema', async () => {
  ...
  try {
    ...
    const parsed = TitleResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'wow.title', err && (err as Error).message);
    throw err;
  }
});
