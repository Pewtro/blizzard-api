...
import { AchievementResponseSchema } from '../generated';

test('wow.achievement should match AchievementResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['achievement'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = AchievementResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'wow.achievement', err && (err as Error).message);
    throw err;
  }
});
