...
import { AccountResponseSchema } from '../generated';

test('d3.account should match AccountResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['account'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = AccountResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'd3.account', err && (err as Error).message);
    throw err;
  }
});
