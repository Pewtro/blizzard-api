...
import { RealmIndexResponseSchema } from '../generated';

test('classic-wow.realmIndex should match RealmIndexResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['realmIndex'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = RealmIndexResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'classic-wow.realmIndex', err && (err as Error).message);
    throw err;
  }
});
