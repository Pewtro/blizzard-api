...
import { ActIndexResponseSchema } from '../generated';

test('d3.actIndex should match ActIndexResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['actIndex'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = ActIndexResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'd3.actIndex', err && (err as Error).message);
    throw err;
  }
});
