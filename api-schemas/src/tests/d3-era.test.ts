...
import { EraResponseSchema } from '../generated';

test('d3.era should match EraResponseSchema', async () => {
  ...
  try {
    ...
    const parsed = EraResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'd3.era', err && (err as Error).message);
    throw err;
  }
});
