...
import { MountIndexResponseSchema } from '../generated';

test('wow.mountIndex should match MountIndexResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['mountIndex'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = MountIndexResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'wow.mountIndex', err && (err as Error).message);
    throw err;
  }
});
