...
import { ToyResponseSchema } from '../generated';

test('wow.toy should match ToyResponseSchema', async () => {
  ...
  try {
    ...
    const parsed = ToyResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'wow.toy', err && (err as Error).message);
    throw err;
  }
});
