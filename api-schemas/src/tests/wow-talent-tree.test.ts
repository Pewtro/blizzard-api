...
import { TalentTreeResponseSchema } from '../generated';

test('wow.talentTree should match TalentTreeResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['talentTree'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = TalentTreeResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'wow.talentTree', err && (err as Error).message);
    throw err;
  }
});
