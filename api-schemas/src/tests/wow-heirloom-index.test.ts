...
import { HeirloomIndexResponseSchema } from '../generated';

test('wow.heirloomIndex should match HeirloomIndexResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['heirloomIndex'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = HeirloomIndexResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'wow.heirloomIndex', err && (err as Error).message);
    throw err;
  }
});
