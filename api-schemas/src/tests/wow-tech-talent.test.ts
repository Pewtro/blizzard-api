...
import { TechTalentResponseSchema } from '../generated';

test('wow.techTalent should match TechTalentResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['techTalent'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = TechTalentResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'wow.techTalent', err && (err as Error).message);
    throw err;
  }
});
