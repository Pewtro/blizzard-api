...
import { LadderSummaryResponseSchema } from '../generated';

test('sc2.ladderSummary should match LadderSummaryResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['ladderSummary'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = LadderSummaryResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'sc2.ladderSummary', err && (err as Error).message);
    throw err;
  }
});
