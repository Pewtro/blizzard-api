...
import { SoulbindIndexResponseSchema } from '../generated';

test('wow.soulbindIndex should match SoulbindIndexResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['soulbindIndex'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = SoulbindIndexResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'wow.soulbindIndex', err && (err as Error).message);
    throw err;
  }
});
