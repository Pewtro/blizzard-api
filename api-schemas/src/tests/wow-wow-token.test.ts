...
import { WowTokenResponseSchema } from '../generated';

test('wow.wowToken should match WowTokenResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['wowToken'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = WowTokenResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'wow.wowToken', err && (err as Error).message);
    throw err;
  }
});
