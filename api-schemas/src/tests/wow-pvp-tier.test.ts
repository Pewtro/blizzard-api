...
import { PvpTierResponseSchema } from '../generated';

test('wow.pvpTier should match PvpTierResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['pvpTier'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = PvpTierResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'wow.pvpTier', err && (err as Error).message);
    throw err;
  }
});
