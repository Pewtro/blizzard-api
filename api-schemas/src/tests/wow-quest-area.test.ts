...
import { QuestAreaResponseSchema } from '../generated';

test('wow.questArea should match QuestAreaResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['questArea'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = QuestAreaResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'wow.questArea', err && (err as Error).message);
    throw err;
  }
});
