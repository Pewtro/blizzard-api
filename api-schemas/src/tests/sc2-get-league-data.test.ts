...
import { GetLeagueDataResponseSchema } from '../generated';

test('sc2.getLeagueData should match GetLeagueDataResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['getLeagueData'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = GetLeagueDataResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'sc2.getLeagueData', err && (err as Error).message);
    throw err;
  }
});
