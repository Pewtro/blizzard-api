...
import { PvpTierIndexResponseSchema } from '../generated';

test('wow.pvpTierIndex should match PvpTierIndexResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['pvpTierIndex'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = PvpTierIndexResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'wow.pvpTierIndex', err && (err as Error).message);
    throw err;
  }
});
