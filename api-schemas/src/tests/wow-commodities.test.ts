...
import { CommoditiesResponseSchema } from '../generated';

test('wow.commodities should match CommoditiesResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['commodities'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = CommoditiesResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'wow.commodities', err && (err as Error).message);
    throw err;
  }
});
