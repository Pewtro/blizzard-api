...
import { LegacyLadderResponseSchema } from '../generated';

test('sc2.legacyLadder should match LegacyLadderResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['legacyLadder'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = LegacyLadderResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'sc2.legacyLadder', err && (err as Error).message);
    throw err;
  }
});
