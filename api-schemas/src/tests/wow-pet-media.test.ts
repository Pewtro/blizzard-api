...
import { PetMediaResponseSchema } from '../generated';

test('wow.petMedia should match PetMediaResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['petMedia'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = PetMediaResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'wow.petMedia', err && (err as Error).message);
    throw err;
  }
});
