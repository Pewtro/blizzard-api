...
import { ItemSetIndexResponseSchema } from '../generated';

test('wow.itemSetIndex should match ItemSetIndexResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['itemSetIndex'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = ItemSetIndexResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'wow.itemSetIndex', err && (err as Error).message);
    throw err;
  }
});
