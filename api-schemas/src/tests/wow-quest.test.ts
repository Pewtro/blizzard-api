...
import { QuestResponseSchema } from '../generated';

test('wow.quest should match QuestResponseSchema', async () => {
  ...
  try {
    ...
    const parsed = QuestResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'wow.quest', err && (err as Error).message);
    throw err;
  }
});
