...
import { ItemSearchResponseSchema } from '../generated';

test('classic-wow.itemSearch should match ItemSearchResponseSchema', async () => {
  ...
  try {
    ...
    const fn = mod['itemSearch'];
    if (typeof fn !== 'function') {
      throw new Error('Endpoint function not found');
    }
    const resource = fn();
    const res = await client.sendRequest(resource);
    const parsed = ItemSearchResponseSchema.safeParse(res);
    if (!parsed.success) {
      console.error(parsed.error.format());
    }
    expect(parsed.success).toBe(true);
  } catch (err) {
    console.warn('Skipping test for', 'classic-wow.itemSearch', err && (err as Error).message);
    throw err;
  }
});
