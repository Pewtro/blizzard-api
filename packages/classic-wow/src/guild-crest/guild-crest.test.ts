...
describe.concurrent('classicGuildCrestApi', () => {
  ...
  test('should return the guild crest border resource for a given borderId', ({ expect }) => {
    const borderId = 123;
  ...
  test('should return the guild crest emblem resource for a given emblemId', ({ expect }) => {
    const emblemId = 456;
