...
describe.concurrent('smoketest some wow api responses', async () => {
  ...
  test('should be able to fetch an achievement', async ({ expect }) => {
    const response = await client.sendRequest(wow.achievement(16_542));

    expect(response).toBeDefined();
  });
  test('should be able to connected realm index', async ({ expect }) => {
  ...
  test('can search for a connected realm', async ({ expect }) => {
