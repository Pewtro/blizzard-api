...
describe.concurrent('talentApi', () => {
  ...
  test('should return the correct path for pvpTalentIndex', ({ expect }) => {
    const resource = pvpTalentIndex();
    expect(resource.path).toBe(`${wowBasePath}/pvp-talent/index`);
    expect(resource.namespace).toBe('static');
  });

  test('should return the correct path for talent', ({ expect }) => {
    const talentId = 456;
  ...
  test('should return the correct path for talentIndex', ({ expect }) => {
    const resource = talentIndex();
    expect(resource.path).toBe(`${wowBasePath}/talent/index`);
    expect(resource.namespace).toBe('static');
  });

  test('should return the correct path for talentTree', ({ expect }) => {
  ...
  test('should return the correct path for talentTreeIndex', ({ expect }) => {
    const resource = talentTreeIndex();
    expect(resource.path).toBe(`${wowBasePath}/talent-tree/index`);
    expect(resource.namespace).toBe('static');
  });

  test('should return the correct path for talentTreeNodes', ({ expect }) => {
    const talentTreeId = 789;
