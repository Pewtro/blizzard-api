...
describe('classic-wow media search integration', async () => {
  ...
  test('performs a media search and validates items', async ({ expect }) => {
    ...
    if (!parsedSearch.success) {
      console.error('Media search validation failed:', prettifyError(parsedSearch.error));
    }
    ...
    if (!parsedEraSearch.success) {
      console.error('Media search validation failed:', prettifyError(parsedEraSearch.error));
    }
